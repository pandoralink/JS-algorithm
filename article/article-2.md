...
这是因为可能存在，左指针或者右指针存在一直比基准值小或者大的情况，这样某次循环中，不加以限制的话，左指针或者右指针可能直接就窜到了数组头部或者数组尾部，注意，左右指针代表基准值的边界，所以最多移动到与左右指针相同的位置，如果此时，左或者右指针没有动过就会导致 left = nums.length 或者 right = -1;
